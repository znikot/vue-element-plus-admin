<template>
  ...
  <!-- switch -->
  <el-switch
    v-if="state.renderer == 'switch'"
    @change="onFieldChanged"
    :model-value="field.toString()"
    :loading="row.loading"
    :active-value="options.active || '0'"
    :inactive-value="options.inactive || '1'"
  ...
  <div v-if="state.renderer == 'image' && field" class="zk-render-image">
    <el-image :hide-on-click-modal="true" :preview-teleported="true" :preview-src-list="[fullUrl(field)]" :src="fullUrl(field)"></el-image>
  </div>
  ...
  <div v-if="state.renderer == 'images'" class="zk-render-image">
    <template v-if="Array.isArray(field) && field.length">
      <el-image
        ...
        :preview-src-list="arrayFullUrl(field)"
        class="images-item"
        :src="fullUrl(item)"
  ...
  <div v-if="state.renderer == 'tag' && field !== ''">
    <el-tag
      :type="options.tagType ? options.tagType[field] ?? 'success' : 'success'"
      :effect="options.tagEffect ?? 'light'"
      :size="options.tagSize ?? 'default'"
    >
  ...
  <div v-if="state.renderer == 'tags'">
    ...
    <template v-else>
      <el-tag class="m-10" v-if="field !== ''" :type="getTagType(field, field.custom)" :effect="field.effect ?? 'light'" :size="field.size ?? 'default'">
  ...
  <div v-if="state.renderer == 'url' && field">
    <el-input :model-value="field" :placeholder="t('Link address')">
      <template #append>
        <el-button @click="typeof field.click == 'function' ? field.click(row, field, field, column, index) : openUrl(field, field)">
  ...
  <div v-if="state.renderer == 'timestamp'">
    {{ !field ? '-' : timestampFormat(field, state.options.format ?? undefined) }}
  </div>
  ...
  <div v-if="state.renderer == 'link'">
    <el-link @click="execProp(props.column, 'renderer.options.clicked', '', props.row)" type="primary">{{ field }}</el-link>
  </div>

  <!-- customTemplate 自定义模板 -->
  <div v-if="state.renderer == 'customTemplate'" v-html="field.customTemplate ? field.customTemplate(row, field, field, column, index) : ''"></div>

  <!-- 自定义组件/函数渲染 -->
  <component
    v-if="state.renderer == 'customRender'"
    ...
    <template v-for="(btn, idx) in options.buttons" :key="idx">
      <template v-if="btn.isVisible ? btn.isVisible(props.row) : true">
        ...
        <el-button
          ...
          :disabled="btn.isEnable && !btn.isEnable(row, field)"
          v-bind="btn.attr"
          link
        >
          <Icon :icon="btn.icon" />
...
const getTagType = value => {
  let tt = ''
  if (props.column.renderer.options && props.column.renderer.options.tagType) {
    tt = props.column.renderer.options.tagType[value]
  }
  return tt || 'success'
}
...
const routerClick = row => {
  ...
  if (!props.column.renderer.options || !props.column.renderer.options.path) return
  let path = ''
  if (typeof props.column.renderer.options.path == 'function') path = props.column.renderer.options.path(row)
  else {
    path = props.column.renderer.options.path
  }
  router.push(path)
}
...
.table-operate .icon {
  ...
  // color: var(--zk-bg-color-overlay) !important;
}
.move-button {
  cursor: move;
}
